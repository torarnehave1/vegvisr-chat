...
// Recognise /graph/<slug> share links generated by the SEO worker so they
// render as a rich card with image + description, instead of falling
// through to a bare link. These carry a slug, not a graphId query param, so
// extractGraphId() above does not match them. The same seo-worker is bound
// to several zones — seo.vegvisr.org itself, plus www.vegvisr.org/graph/*
// and the universi.no equivalents (see seo-worker/wrangler.toml routes) —
// so all of them resolve through the same /graph/{slug}?format=json lookup.
const SEO_GRAPH_HOSTS = new Set([
  'seo.vegvisr.org',
  'www.vegvisr.org',
  'vegvisr.org',
  'universi.no',
...
function extractSeoGraphSlug(url: string): string | null {
  try {
    const u = new URL(url)
    if (!SEO_GRAPH_HOSTS.has(u.hostname)) return null
    const match = u.pathname.match(/^\/graph\/([^/]+)\/?$/)
...
interface TextPart {
  type: 'text' | 'link' | 'graph' | 'youtube' | 'instagram' | 'seo-graph' | 'recording' | 'image' | 'pdf'
  value: string
  graphId?: string
  youtubeId?: string
    ...
    <div id={`msg-${message.id}`} className={`flex ${isOwn ? 'justify-end' : 'justify-start'} mb-1.5 group transition-all`}>
      ...
      >
        ...
        {msgType === 'text' && message.body && (() => {
          ...
          const richCards = parts.filter(p => p.type === 'graph' || p.type === 'youtube' || p.type === 'instagram' || p.type === 'seo-graph' || p.type === 'recording' || p.type === 'image' || p.type === 'pdf' || p.type === 'link')
          return (
            <div>
