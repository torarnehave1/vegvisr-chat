...
// Recognise seo.vegvisr.org/graph/<slug> share links (generated by the SEO
// worker) so they render as a rich card with image + description, instead of
// falling through to a bare link. These carry a slug, not a graphId query
// param, so extractGraphId() above does not match them.
function extractSeoGraphSlug(url: string): string | null {
  try {
    const u = new URL(url)
    if (u.hostname !== 'seo.vegvisr.org') return null
