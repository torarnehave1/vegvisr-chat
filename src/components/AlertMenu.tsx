...
export function AlertMenu({ groupId, auth, currentUserId }: Props) {
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-72 max-h-96 overflow-y-auto bg-slate-800 border border-white/10 rounded-xl shadow-xl z-50 p-2">
          <div className="px-2 py-1.5 text-xs uppercase tracking-wider text-white/40">Email alerts</div>
          {loading ? (
            <div className="px-2 py-3 text-sm text-white/40">Loading...</div>
          ) : !isOwner ? (
            <div className="px-2 py-3 text-sm text-white/40">Only the group owner can send email alerts.</div>
          ) : alertable.length === 0 ? (
            <div className="px-2 py-3 text-sm text-white/40">No members have enabled email alerts yet.</div>
          ) : (
            <div className="space-y-1">
              {alertable.map(m => {
                ...
                return (
                  <div key={m.user_id} className="flex items-center gap-2.5 px-2 py-1.5 rounded-lg hover:bg-white/5">
                    <div className="w-7 h-7 rounded-full bg-white/10 flex items-center justify-center text-white/50 text-xs overflow-hidden flex-shrink-0">
