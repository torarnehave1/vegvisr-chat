    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-white/10">
        ...
        <h2 className="text-sm font-semibold uppercase tracking-[0.2em] text-white/70 flex-1 truncate">
          Questions <span className="text-white/30 normal-case tracking-normal">— {groupName}</span>
        </h2>
        {auth && (
          <button
            ...
            className={`text-xs font-semibold px-3 py-1.5 rounded-lg transition-colors ${
              showForm
                ...
                : 'bg-gradient-to-r from-amber-500 to-sky-500 text-white shadow-lg shadow-amber-500/20'
            }`}
          >
            {showForm ? 'Cancel' : '+ Ask'}
          </button>
      ...
      {showForm && auth && (
        <div className="border-b border-white/10 bg-white/[0.03] px-4 py-4 space-y-3">
          <input
            ...
            maxLength={120}
            className="w-full rounded-xl border border-white/10 bg-slate-900/60 px-3 py-2 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-amber-400/50"
          />
          <textarea
            ...
            rows={3}
            className="w-full rounded-xl border border-white/10 bg-slate-900/60 px-3 py-2 text-sm text-white placeholder:text-white/30 resize-none focus:outline-none focus:border-amber-400/50"
          />
          <div className="flex items-center justify-end">
            <button
              onClick={handleSubmit}
              disabled={!formTitle.trim() || !formDescription.trim() || submitting}
              className="px-4 py-1.5 bg-amber-600 text-white text-xs font-semibold rounded-lg disabled:opacity-40 hover:bg-amber-500 transition-colors"
            >
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="text-sm font-semibold text-white/90">{question.label}</h3>
                    <span
                  ...
                  {meta.answer && !isAnswering && (
                    <div className="mt-3 rounded-lg border-l-2 border-emerald-400/60 bg-emerald-500/5 px-3 py-2">
                      ...
                      <div className="text-xs text-white/80 leading-relaxed whitespace-pre-line">
                  ...
                  {isAnswering && isOwner && (
                    <div className="mt-3 space-y-2">
                      <textarea
                        ...
                        rows={3}
                        className="w-full rounded-xl border border-emerald-400/30 bg-emerald-500/5 px-3 py-2 text-sm text-white placeholder:text-white/30 resize-none focus:outline-none focus:border-emerald-400/60"
                      />
                      <div className="flex items-center gap-2 justify-end">
                        ...
                        <button
                          ...
                          className="px-3 py-1 bg-emerald-600 text-white text-xs font-semibold rounded-lg disabled:opacity-40 hover:bg-emerald-500 transition-colors"
                        >
                    ...
                    {auth && (
                      <button
                        ...
                        className={`flex items-center gap-1 text-xs transition-colors ${
                          hasVoted ? 'text-amber-400 cursor-default' : 'text-white/30 hover:text-amber-400'
                        }`}
